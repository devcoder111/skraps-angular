...
export class PortfolioPageComponent implements OnDestroy {
  ...
  public portfolioEmitter$ = new BehaviorSubject<any>(this.userPortfolios);
  public userRiskInfo;
  public userRiskInfoEmitter$ = new BehaviorSubject<any>(this.userRiskInfo);
  public startInvestment:boolean = false;
  ...
  public setPortfolio(value: IPortfolio) {
    this.startInvestment = true;
    this.store.dispatch(new SetPortfolio(value));
    setTimeout(function(){
      let el = document.getElementById('choosePortfolio');
      el.scrollIntoView();
    },500);
  }
  ...
  getUsersPortfolios() {
    this.portfolioService.userPortfoliosByRisk()
    .subscribe((result) => {
      if (result.data.length > 0) {
        this.userPortfolios = result.data;
        this.portfolioEmitter$.next(this.userPortfolios);
      }
    });
  }

  getUserRiskInfo() {
    this.portfolioService.userRiskInfo()
    .subscribe((result) => {
      if (typeof result.data != 'undefined') {
        this.userRiskInfo = result.data;
        this.userRiskInfoEmitter$.next(this.userRiskInfo);
      }
    });
  }

  goBack() {
    this.startInvestment = false;
  }
