...
export class CardService {
  ...
  private handleError(error: HttpErrorResponse) {
    ...
    //   newob.snackBar.open(error.error.error, 'error', {
    //     duration: 2000,
    //  });
    newob.openSnackBar(true , error.error.error);
    }
    return throwError(
      'Something bad happened; please try again later.');
  };

  public openSnackBar(isError = false, text?: string) {
  ...
  connectedAccounts(): Observable<any> {
    return this.http.post(`${environment.baseUrl}/connected-accounts`, {});
  }
}
